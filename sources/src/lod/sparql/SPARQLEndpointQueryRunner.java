...
public class SPARQLEndpointQueryRunner extends AsyncRunner implements
		...
		while (true) {
			try {
				...
				if (!ex.getMessage().contains("Read timed out")) {
					// WE SHOULD PROBABLY NOT DO THIS, BUT A LOT OF PROCESSES
					// ARE FAILING BECAUSE THE RESULTS ARE INCORRECTLY escaped
					// throw new OperatorException(
	...
	public ResultSet runSelectQuery(String query) throws OperatorException {
		...
		while (true) {
			try {
				ResultSet resultstmp = objectToExec.execSelect();
				results = ResultSetFactory.copyResults(resultstmp);
				objectToExec.close();
				break;
			} catch (Exception ex) {
				if (!ex.getMessage().contains("Read timed out"))
