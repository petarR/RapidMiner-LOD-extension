...
public abstract class BaseGenerator extends Operator {
	 ...
	 */
	protected abstract String getSPARQLQueryOutgoing(String uri);
	 ...
	 * @return {@link String} object.
	 */
	protected abstract String getSPARQLQueryIncoming(String uri);
		...
		if (generator != Generators.DATA_PROPERTIES) {
			for (String attrName : uniqueAttributesToAdd) {
				...
				if (addedAttributeOverall.containsKey(attrName)) {
					...
					addAtribute(attributeNamePrefix + nameSeparator + attrName,
							ontology, exampleSet);
				}
				int h = 0;
	...
	public void processFinished() throws OperatorException {
		...
		Boolean emptycache = Boolean
				.parseBoolean(ParameterService
						.getParameterValue(PluginInitLOD.PROPERTY_RMLOD_QUERY_CACHE_REMOVE_AFTER_PROCESS));
		if (emptycache)
		...
		emptycache = Boolean
				.parseBoolean(ParameterService
						.getParameterValue(PluginInitLOD.PROPERTY_RMLOD_MODEL_REMOVE_AFTER_PROCESS));
		if (emptycache)
