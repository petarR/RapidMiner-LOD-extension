...
public class HierarchyNodeRenderer<V, E> implements Renderer.Vertex<V, E> {
	...
	private HierarchyGraphCreator graphCreator;
	private OntologyHierarchy model;

	private int maxLeafSize = 0;

	public HierarchyNodeRenderer(HierarchyGraphCreator graphCreator,
			int maxLeafSize) {
	...
	protected boolean vertexHit(RenderContext<V, E> rc, Shape s) {
		...
		if (deviceRectangle != null) {
			MutableTransformer vt = rc.getMultiLayerTransformer()
	...
	protected void paintShapeForVertex(RenderContext<V, E> rc, V v, Shape shape) {
		...
		// * (FREQUENCY_BAR_MAX_HEIGHT - FREQUENCY_BAR_MIN_HEIGHT)
		// + FREQUENCY_BAR_MIN_HEIGHT;
		// double width = shape.getBounds().getWidth() - 2
		// * FREQUENCY_BAR_OFFSET_X - 1;
		// double xPos = shape.getBounds().getX() + FREQUENCY_BAR_OFFSET_X;
		// double yPos = shape.getBounds().getY()
		// + shape.getBounds().getHeight() - FREQUENCY_BAR_OFFSET_Y
		// - height;
		// ColorProvider colorProvider = new ColorProvider();
		// for (String labelValue : countMap.keySet()) {
		// int count = tree.getCount(labelValue);
		// double currentWidth = ((double) count / (double) frequencySum)
		// * width;
		// Rectangle2D.Double frequencyRect = new Rectangle2D.Double(xPos,
		// yPos, currentWidth, height);
		// int counter = model.getTrainingHeader().getAttributes()
		// .getLabel().getMapping().mapString(labelValue);
		// g.setColor(colorProvider.getPointColor((double) counter
		// / (double) (numberOfLabels - 1)));
		// g.fill(frequencyRect);
		// g.setColor(Color.BLACK);
		// xPos += currentWidth;
		// }
		// g.setColor(Color.BLACK);
		// g.draw(new Rectangle2D.Double(shape.getBounds().getX()
		// + FREQUENCY_BAR_OFFSET_X, yPos, width, height));
		//
		// g.setPaint(oldPaint);
		// }
	}
